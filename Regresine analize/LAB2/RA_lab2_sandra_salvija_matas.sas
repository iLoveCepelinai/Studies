proc import out = work.wine_all
			datafile = "/home/u45449192/SAS ODfA (new)/Regresine analize/WineQT.csv" 
    replace;
    getnames = yes;
run;
...
/* proc genmod neskaiciuoja vif, todel panaudojome proc reg. Atsakymai panasus i tai,
   ka gavome su R. */
proc reg data=work.wine2 simple;
	model alcohol = 'fixed acidity'n 'volatile acidity'n  
   					'citric acid'n 'residual sugar'n 
   					'chlorides'n 'free sulfur dioxide'n
   					'total sulfur dioxide'n density
...
proc genmod data = work.wine3 plots=(cooksd stdreschi);
   ...
   model alcohol = 'volatile acidity'n  
                  ...
                  link=power(-1)
                  type3;
    output predicted = spetos_reiks; 
run;

/* Kvantiliu grafikas */
proc univariate data=work.data1;
   qqplot spetos_reiks;
run;
...
/* Kvantiliu grafikas */
proc univariate data=work.data2;
   qqplot spetos_reiks;
run;